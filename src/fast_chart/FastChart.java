...
public class FastChart extends JPanel{
    ...
    private void plot(Graphics g, int width, int height,
        ...
        for(int i = 0; i < size; i++) {
            ...
            XY[] points = graphics.get(i);
            
            int[] xs = new int[points.length]; 
            for(int j = 0; j < xs.length; j++) {
                xs[j] = paddingLeft + (int)convertXToScreenPx(widthOfPlot, points[j].x);
            }
            
            int[] ys = new int[points.length];
            for(int j = 0; j < ys.length; j++) {
                ys[j] = paddingTop + (int)convertYToScreenPx(heightOfPlot, points[j].y);
            }
    ...
    private void plotArea(Graphics g, int width, int height,
        ...
        for(int i = 0; i < size; i++) {
            ...
            xs[xs.length - 2] = paddingLeft + (int)convertXToScreenPx(widthOfPlot, points[xs.length - 3].x);
            xs[xs.length - 1] = paddingLeft + (int)convertXToScreenPx(widthOfPlot, points[0].x);
